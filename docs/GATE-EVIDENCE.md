...
3. **smoke vs example.com (200) / smoke vs known-404 expecting 200** —
   ...
   (`self-test-red-path.yml`, `https://raw.githubusercontent.com/ynetplus/orbit/main/definitely-does-not-exist-file-xyz.txt`
   — a reliably-404 path on GitHub's own raw-content CDN, not a
   third-party status-code test service — with `expected_status: "200"`,
   FAILS loudly after the configured retries).
